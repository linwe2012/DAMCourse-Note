...
在使用传统算法统计图像特征的时候，我们可以根据图像的颜色特征、纹理、边缘特征等信息，可以实现一定的任务，如基于HoG和SVG或基于Harr与AdaBoost的人脸与人脸特征点的检测算法，其结果与当前盛行的深度学习的方法相差无几，但是传统算法的问题在于效率上，同样的检测任务传统算法可能一帧需要一分钟，而深度学习算法可以做到实时检测。